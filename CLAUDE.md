...
## Current phase
Week 2 COMPLETE as of 2026-07-10 (Week 1 committed as `07159cd`). Shipped: student bulk provisioning (CSV upload → Zod row validation → CSPRNG temp passwords → BullMQ-queued credential emails with delivery tracking in `student_provisioning_log`), lecturer self-registration + System Admin approval workflow (login gated on approval status, enumeration-safe), audit log middleware on all writes (payload hashed, never stored). Notable deviations from spec: BullMQ instead of Bull (Bull is in maintenance mode), Prisma pinned to v6 (v7 dropped `datasource url` schema syntax), added `LECTURER` base role (Supervisor/Evaluator/HeadJudge are CPI-scoped, assigned via junction tables in Week 3), ioredis pinned to exactly 5.10.1 to match BullMQ's pinned dep. SMTP is env-driven with a dev jsonTransport fallback — real credentials needed before pilot. 8 tests green, typecheck+lint clean. **Week 2 not yet committed.** Next up: Week 3 — CPI creation, timeline engine (10 phases), phase-gating middleware, supervisor addition (mode determination), evaluator/Head Judge assignment.
